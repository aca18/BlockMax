...
def  main():
  ...
  installer = "\installer\BlockMax Installer.exe"
   
  if demo:
    installer = "\installer\BlockMax Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BlockMax manual.pdf" 
  ]
  ...
  ZIPNAME = "BlockMax-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BlockMax-v" + FULLVERSIONSTR + "-win-demo.zip"
